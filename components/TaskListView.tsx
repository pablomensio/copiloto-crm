...
import { CheckCircle, Circle, Clock, AlertTriangle, Calendar, Filter, Plus, User, Phone } from 'lucide-react';
...
const TaskListView: React.FC<TaskListViewProps> = ({ tasks, leads, onTaskToggle, onAddTaskClick }) => {
            ...
            <div className={`group flex items-center p-4 bg-white border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors ${task.isCompleted ? 'opacity-60' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        {lead && (
                            <span className="flex items-center gap-1 px-1.5 py-0.5 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded border border-indigo-100">
                ...
                <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${task.priority === 'High' ? 'bg-red-100 text-red-700' :
