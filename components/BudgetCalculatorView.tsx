...
const BudgetCalculatorView: React.FC<BudgetCalculatorViewProps> = ({
    ...
    const [comoSeCubre, setComoSeCubre] = useState({
        ...
        pesos: initialBudget?.items?.pesos ?? 0,
        sena: initialBudget?.items?.sena ?? 0,
        credito: initialBudget?.items?.credito ?? 0,
    });
    ...
    useEffect(() => {
        if (initialBudget) {
            setACubrir({
                valorVehiculo: initialBudget.items?.valorVehiculo ?? 0,
                transferencia: initialBudget.items?.transferencia ?? 0,
                costoOtorgamiento: initialBudget.items?.costoOtorgamiento ?? 0,
            });
            setComoSeCubre({
